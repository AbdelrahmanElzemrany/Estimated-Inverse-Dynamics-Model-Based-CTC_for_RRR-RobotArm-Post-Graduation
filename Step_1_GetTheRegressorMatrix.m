...
robot.nbDOF = 3;                                                        % Number of Degrees Of Freedom (DOF)
...
robot.jointType = ['revol';'revol';'revol'];    % Type of the robot joints: either 'revol' or 'prism'                        
...
robot.dhParameter.theta = [robot.symbolicParameters.Q(1); robot.symbolicParameters.Q(2); robot.symbolicParameters.Q(3)];
robot.dhParameter.d     = sym([0.6;   0;   0]);  % <-- Fixed: 0.6 goes to d1, d2 is 0
robot.dhParameter.a     = sym([  0; 0.0; 0.5]);  % <-- Fixed: Link lengths along X-axes
robot.dhParameter.alpha = sym([  0; pi/2;  0]);  % Twist angles   
...
robot.numericalParameters.numParam=[12;12;12];                 %If you have 10 parameters per link(no friction case) change the number 12 to 10
...
for i = 1:samples
    ...
    Obs(robot.nbDOF*(i-1)+1:robot.nbDOF*i,:) = Y_gg_handle(gx,gy,gz,q1,q2,q3,qp1,qp2,qp3,qpp1,qpp2,qpp3);
end
